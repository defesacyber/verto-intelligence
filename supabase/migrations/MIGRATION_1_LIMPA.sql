-- ============================================
-- MIGRATION 1: Market Data Tables (VERSÃO LIMPA E RECRIA)
-- ============================================
...
DROP POLICY IF EXISTS "Allow read access to neighborhoods" ON public.neighborhoods;
DROP POLICY IF EXISTS "Allow read access to market data" ON public.market_data_cache;
DROP POLICY IF EXISTS "Allow read access to time series" ON public.market_time_series;
DROP POLICY IF EXISTS "Allow read access to macro indicators" ON public.macro_indicators;
DROP POLICY IF EXISTS "Authenticated users can view cities" ON public.cities;
DROP POLICY IF EXISTS "Authenticated users can view neighborhoods" ON public.neighborhoods;
DROP POLICY IF EXISTS "Authenticated users can view market data" ON public.market_data_cache;
DROP POLICY IF EXISTS "Authenticated users can view time series" ON public.market_time_series;
DROP POLICY IF EXISTS "Authenticated users can view macro indicators" ON public.macro_indicators;
DROP POLICY IF EXISTS "Service role can manage market data" ON public.market_data_cache;
DROP POLICY IF EXISTS "Service role can manage time series" ON public.market_time_series;
DROP POLICY IF EXISTS "Service role can manage macro indicators" ON public.macro_indicators;
...
-- Criar políticas (agora que limpamos as antigas)
...
DROP TRIGGER IF EXISTS update_market_data_cache_updated_at ON public.market_data_cache;
